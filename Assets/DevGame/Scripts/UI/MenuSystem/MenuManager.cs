...
public class MenuManager : MonoBehaviour
{
    ...
    public void SetParentForDialog(Transform tr, string name)
    {
        foreach (var pa in nameParent)
        {
            if (pa.name.Equals(name))
            {
                tr.transform.parent = pa;
                break;
            }
        }
    }    
    ...
    public void OpenDialog(Dialog instance)
    {
        // De-activate top menu
        var maxSortingOrder = 0;
        var topCanvas = instance.GetComponentInChildren<Canvas>();
        //        dialogs.Remove(instance);
        if (dialogs.Count > 0)
        {
            foreach (var dialog in dialogs)
            {
                if (dialog == null || dialog.GetComponentInChildren<Canvas>() == null)
                {
                    continue;
                }

                var sortingOrder = dialog.GetComponentInChildren<Canvas>().sortingOrder;
                if (sortingOrder > maxSortingOrder)
                    maxSortingOrder = sortingOrder;
            }
        }
        ...
        if (menuStack.Count > 0)
        {
            var topMenuCanvas = menuStack.Peek().GetComponentInChildren<Canvas>();
            if (topMenuCanvas != null && topMenuCanvas.sortingOrder > maxSortingOrder)
                maxSortingOrder = topMenuCanvas.sortingOrder;
        }
        ...
        for (var i = 0; i < dialogs.Count; i++)
        {
            try
            {
                ...
                if (topMenu == null && dialogs.Count == 0)
                    return;
                var maxSortingOrder =
                    (topMenu == null && dialogs[0] != null && dialogs[0].GetComponentInChildren<Canvas>() != null)
                        ? dialogs[0].GetComponentInChildren<Canvas>().sortingOrder
                        : topMenu.GetComponentInChildren<Canvas>().sortingOrder;
                foreach (var dialog in dialogs)
                {
                    if (instance == dialog || dialog == null || dialog.GetComponentInChildren<Canvas>() == null)
                        continue;
                    if (maxSortingOrder < dialog.GetComponentInChildren<Canvas>().sortingOrder)
                    {
                        topDialog = dialog;
                        topMenu = null;
                    }
